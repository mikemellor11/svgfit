(function(exports) {
	...
	exports.fit = function( input, output, opts ){
		opts = opts || {};
		...
		if( typeof input === "string" && fs.existsSync( input ) && fs.lstatSync( input ).isDirectory()){
			...
			files = input.map(function(d){
				return path.resolve(d);
			});
		} else {
			throw new Error( "Input must be Array of files, a String pointing to a single file or String that is a directory" );
		}

		// Remove DS_Stores from files array
		files = files.filter(function(d, i){return d.indexOf('.DS_Store') === -1;});
